...
Pour considérer la dimension temporel et détecter les attaques d'inondation ou à plusieurs étapes, il faut intégrer l'information temporel à notre détection et donc considérer les graphs dynamiques. Une solution serait d'utiliser les graphs dynamiques discrets et c'est d'ailleurs l'approche qu'emploie GSAD \cite{gsad}. %Cette façon de gérer les graphes dynamiques est facile à comprendre et à implémenter, car les captures à intervalle régulier de l'état du graphe forment une séquence de graphs indépendants sur lesquels on peut appliquer les GNN traditionnels sans modification majeure. Néanmoins, leur principal inconvénient est qu'ils sont très lourds et peu efficaces pour détecter les dépendances sur de longues périodes.
