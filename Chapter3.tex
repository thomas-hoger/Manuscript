...
La figure \ref{fig:nlp_process} présente comment on applique le NLP à notre problème pour récupérer un embedding associé à un mot. Tout d'abord on va aller voir le fichier json qui contient les données qu'on a extrait des paquets et on va transformer ces paquets en représentation lexicographiques pouvant être donnée à un modèle de NLP, une phrase. Pour ça on commence par nettoyer les chaines en enlevant les caractères comme les tirets et les points au sein des mots. ensuite on va concaténer ces mots entre eux, séparés par des espaces. on va par contre séparé les infos en plusieurs phrase différentes selon le type de couche où on récupére l'info. pour les protocole on va donner le nom dans la phrase pour ajouter du contexte aux mots. on va donc avoir une phrase pour les infos générales comme les addresses puis une phrase par protocole. le modèle va quand même considérer le paquet, donc les plusieurs phrases comme un ensemble mais il va aussi prendre en compte la séparation entre les phrases. 

Ensuite comme pour du NLP classique on va appliquer des tokenization qui vont essayer de récupérer des mots déjà connus et si ils les trouvent pas ils essaient de découper le mot en sous-unité. Sur la figure on voit notamment que \textit{requester} est découpé en \textit{request} et \textit{##er}, le second signalé comme un suffixe grâce aux signes dièse. Pareil on voit que le terme de \textit{nf} étant inconnu, le tokenizer le découpe en \textit{n} et \textit{##f}. 

Ces tokens sont ensuite transformés en token ids qui peuvent être traités par un modèle.  Ce dernier va retourner des vecteurs, par exemple de taille [768] pour BERT qui représenteront chaque token. Comme on a souvent plusieurs tokens par mots on va vouloir les aggréger et pour ça le plus simple c'est de faire une moyenne des embeddings des tokens d'un mot. 
