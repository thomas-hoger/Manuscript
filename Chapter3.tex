...
Le premier ajustement consiste à supprimer l’information temporelle explicite du modèle. Concrètement, cela se traduit par le retrait du terme \( \Delta t \) dans la formulation des messages. Dans notre contexte, l’information temporelle est déjà implicitement capturée par les séquences d’interactions lorsqu'elle est traité par le RNN. Le terme \( \Delta t \) est principalement pertinent pour identifier d’éventuelles pauses ou irrégularités dans le trafic. Toutefois, dans le cadre de la détection d’anomalies, cette information ne constitue pas un indicateur suffisamment représentatif d’une attaque. Au contraire, elle peut introduire un biais de surapprentissage susceptible d’être exploité par un attaquant. Pour cette raison, nous avons choisi de retirer cette composante temporelle de la formulation des messages.
...
%Ces ajustements correspondent bien aux spéficités de notre cas d'utilisation et nous les avons donc appliqués. Néanmoins d'autres idées qu'on a pas mis en place mais qu'on juge tout de même intéressant de mentionner.

\subsubsection{Problèmes ouverts}
% enfin, y'a des idées qu'on a pas implémenté parce qu'on considère soit que c'est pas utile mais qu'on considère intéressant de mentionner ici
% , ainsi que la réduction de la complexité du graphe par une transformation en \textit{line graph}

% est-ce qu'on fusionne les réponses avec les questions des requetes ? et tous les messages entre le deux (image wireshark+graph)
% on a décidé que non parce que 

% représentation des flux
% comme les imbrications, ça pourrait être intéressant mais ça fait modifier le graph et l'information vaut pas la peine

% mélanger avec des infos systeme
%  on voulait encoder avec bert+gmm comme baguette et le papier d'avant

% Line graph
% EGAT
% photo "line graph problem" dossier photo
