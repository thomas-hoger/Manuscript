...
Pour considérer la dimension temporel et détecter les attaques d'inondation ou à plusieurs étapes, il faut intégrer l'information temporel à notre détection et donc considérer les graphs dynamiques. Une solution serait d'utiliser les graphs dynamiques discrets, c'est d'ailleurs l'approche qu'emploie GSAD \cite{gsad}. Cette façon de gérer les graphes dynamiques est facile à comprendre et à implémenter, car les captures à intervalle régulier de l'état du graph forment une séquence de graphs indépendants sur lesquels on peut appliquer les GNN traditionnels sans modification majeure. Néanmoins, leur principal inconvénient est qu'ils sont très lourds et peu efficaces pour détecter les dépendances sur de longues périodes.
...
% PCA T-SNE
Pour évaluer la qualité de cette structuration, deux approches complémentaires sont généralement utilisées et reposent sur une interprétation humaine. La première consiste à examiner, au cas par cas, les voisins les plus proches d’un mot donné dans l’espace d’embedding. La seconde consiste à projeter cet espace de haute dimension dans un espace à deux dimensions afin d’en permettre une visualisation globale. Les méthodes les plus couramment utilisées pour cela sont la PCA et le t-SNE, qui représentent respectivement une projection linéaire conservant la structure globale, et une projection stochastique visant à préserver les similarités locales. La figure \ref{fig:pca_simple} illustre un exemple de projection PCA d’un vocabulaire générique encodé avec FastText que nous allons présenter ensuite.

% One Hot
Nous notre but c'est de pouvoir encoder des mots individuellement. Pour ça la méthode la plus simple c'est de faire du one-hot encoding, c'est à dire d'associer à chaque mot à une dimension d'un vecteur. Chaque mot est donc représenté par un vecteur orthogonal. Le fait d'avoir des embeddings unique à chaque mot laisse au GNN suivant la responsabilité de leur donner de la sémantique. Un inconvénient c'est on part de 0 et on pourrait réfléchir à donner un petit coup de pouce au GNN en donnant un peu de base sémantique aux paramètres avec du NLP. L'autre inconvénient majeur est que pour avoir des vecteurs orthogonaux il faut que la dimension de l'espace d'embedding égale à la taille du vocabulaire.
...
% Word2Vec 
Les approches de deep learning permettent justement d’apprendre la sémantique des mots à partir de leur contexte d’utilisation. L’idée principale est qu’un mot peut être compris en observant les mots qui l’entourent dans une phrase ou un corpus. C'est ce que fait Word2Vec\cite{word2vec} qui est constitué de deux modèles qu'il entraine en parallèle, un Continuous BoW (CBOW) qui vise à prédire un mot à partir de son contexte et un Skip-Gram qui, de façon symétrique, vise à prédire les mots du contexte en connaissant un mot en entrée. 
...
% FastText
FastText \cite{fasttext} est justement une extension de Word2Vec qui cherche à découper un mot en sous-unités morphologiques. Contrairement à ce Word2vec, FastText ne considère pas un mot comme une entité atomique, mais le représente comme un ensemble de \textit{n-grammes} de caractères. Cette segmentation permet de partager des informations entre mots présentant des structures similaires. Dans notre cas, cela permet par exemple de décomposer \textit{servingNfId} en plusieurs sous-unités telles que \textit{serving}, \textit{Nf} et \textit{Id}. Chacun de ces segments peut alors porter une information sémantique propre et être réutilisé pour représenter d’autres paramètres partageant des sous-parties communes. Cependant, une limite importante de cette approche réside dans le caractère statique des embeddings. Chaque mot et n-gramme possède une représentation fixe, indépendante du contexte dans lequel il apparaît.

% GPT & BERT
Or, un mot peut être polysémique et donc avoir une sémantique variable en fonction de son contexte d’apparition. Afin de prendre en compte cette ambiguïté sémantique, on a vu l'arrivée de travaux utilisant des architectures de type Transformer, qui permettent d’apprendre des représentations contextuelles dynamiques. Ces modèles, tels que GPT \cite{gpt} et BERT \cite{bert}, utilisent des mécanismes d’attention pour modéliser les dépendances à long terme dans les séquences de texte, ce qui améliore significativement la qualité des représentations apprises. Le premier est basé sur un décodeur qui va analyser le contexte historique d'un mot pour prédire sa suite tandis que le second va prendre le contexte bidirectionnel et sera plus adapté à des tâches de compréhension du langage. Ces modèles sont la plupart du temps pré-entraînés sur de très grands corpus de texte, puis peuvent être adaptés à des tâches spécifiques par le biais d’une phase de fine-tuning. Cependant, leur complexité et leurs besoins en ressources computationnelles sont considérablement plus élevés que les modèles précédents.

% Conclusion
Parmi les approches proposées, GPT, BERT et leurs variantes respectives figurent parmi les modèles obtenant généralement les meilleures performances. Néanmoins, il convient de souligner que ces architectures ont été conçues pour des données textuelles naturelles et reposent sur des hypothèses implicites liées à la structure du langage humain. Dans le cadre de ce travail, le contexte considéré est sensiblement différent. Le vocabulaire manipulé est très spécifique, parfois constitué d’identifiants ou de chaînes non lexicalisées, et les paquets qui les contiennent diffèrent fortement de la structure d’une phrase naturelle. Dès lors, l’utilisation directe de modèles aussi complexes peut s’avérer disproportionnée au regard de la nature des données. Il apparaît donc nécessaire d’évaluer dans quelle mesure ces architectures peuvent être adaptées à ce type de données, et d’identifier les modèles offrant le meilleur compromis entre expressivité, robustesse et coût computationnel dans un contexte aussi spécifique.

\subsection{Avantages, limites et opportunités d'application}

Le fait d’obtenir une proximité géométrique entre des éléments présentant une similarité sémantique constitue une propriété particulièrement intéressante dans notre contexte. En effet, de nombreux paramètres présentent une forte redondance lexicale ou fonctionnelle, comme \textit{servingNfId} et \textit{nf-id}. Ces paramètres peuvent ainsi être associés à des représentations proches dans l’espace latent, ce qui contribue à réduire la complexité du vocabulaire. De plus, l’utilisation de techniques issues du traitement automatique du langage naturel (NLP) pour enrichir la représentation sémantique des vecteurs d’arêtes peut fournir une information inductive supplémentaire au GNN, facilitant ainsi l’apprentissage de relations complexes.

.% Néanmoins, cette propriété introduit également une limite importante, dans la mesure où certaines similarités sémantiques ne doivent pas nécessairement se traduire par une proximité dans l’espace de représentation. Par exemple, des paramètres tels que \textit{source-ip} et \textit{destination-ip} jouent des rôles distincts au sein de la structure réseau et doivent être explicitement différenciés afin de permettre la détection d’anomalies topologiques pertinentes. Ca devrait aller 

Pour réaliser l’embedding d’éléments textuels à l’aide de techniques de NLP, deux approches principales peuvent être envisagées. Utiliser direcectement des modèles pré-entraînés ou réaliser un fine-tuning pour l'adapter à notre vocabulaire. Dans notre cas, les paramètres à encoder correspondent davantage à des noms de variables qu’à des mots issus d’un langage naturel. Si un modèle pré-entrainé peut fonctionner pour certains termes proches du langage courant, tels que \textit{address}, il atteint rapidement ses limites pour des identifiants techniques comme \textit{avType} ou \textit{kausf}, ainsi que pour leurs sous-composants, qui ne correspondent pas à des unités lexicales connues. Il devient donc fondamentale de passer par une  phase de fine-tuning.



% VERIF LES 3 DERNIERES
Les principaux modèles de NLP apprennent la sémantique des mots à partir de leur contexte au sein de phrases. Or, dans notre cas, nous ne disposons pas naturellement de corpus sous cette forme. Il est toutefois possible de représenter les paquets comme des phrases et les paramètres comme des mots. Toutefois, cette approche reste imparfaite, car elle introduit implicitement une notion d’ordre et de position dans la phrase, qui n’a pas de signification dans notre cas. En effet, l’ordre des paramètres dans un paquet et leur proximité ne portent pas d’information pertinente. 

Dans notre contexte, il est plus pertinent de considérer la compréhension d’un paramètre à travers les autres paramètres présents dans le même paquet, ce qui est précisément ce que permet déjà un GNN via les mécanismes de convolution et d’agrégation topologique. L’apport du NLP serait alors principalement complémentaire, en fournissant une initialisation sémantique aux représentations.

Enfin, la principale difficulté liée à l’utilisation du NLP réside dans l’évaluation de ces représentations, qui n’est pas directement interprétable visuellement en haute dimension. Il sera donc nécessaire de recourir à une phase de validation empirique, qui sera détaillée dans le chapitre suivant.

%(montrer PCA des deux)

% Le fait de faire du NLP permet 1) d'ajouter un peu de sémantique inital et 2) de considérer tous les paramètres. 
% 1) Mais la sémantique sera déjà gérée par le GNN donc pas forcément utile (et en plus ça peut induire des biais qu'on pourra pas forcément check vu que subjctif et pas de méthode de vérif auto)
% 2) Sur tous les paramètres y'en a certains qui sont très rares et si on les à tous sans les pondérer d'une certaine manière bah on va se retrouver avec un espace d'embedding très bruité et pas forcément pertinent. 


% le probleme surtout c'est que nlp trop semblable gnn, fait le même taff et lui vole le boulot. on se retruve avec des embeddings moins controlés et moins vérifiables qui vont repasser dans une moulinette qui fait la même chose

%(faire l'état des lieux des méthodes de NLP et voir si y'en a pas certaiens qui font pas avec le contexte). REsttester aussi pour voir si le nlp galère vraiment ou pas a faire un embedding space cohérent avec les parametres qu'on a gardé.

% Dans le cadre de la détection d’attaques connues, lorsque l’on souhaite conserver une certaine interprétabilité des résultats, il est possible de se limiter à un sous-ensemble de paramètres jugés pertinents pour identifier les attaques déjà observées. C’est l’approche retenue dans cette thèse.
