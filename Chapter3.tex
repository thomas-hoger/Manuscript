...
\section{Encodage des paramètres de dimension non-fini via NLP}
...
La figure \ref{fig:nlp_process} illustre le processus d’application du NLP à notre problème afin d’obtenir des embeddings associés aux mots. Dans un premier temps, nous exploitons le fichier JSON contenant les données extraites des paquets, que nous transformons en phrases. Pour cela, les chaînes de caractères sont nettoyées en supprimant certains caractères spéciaux, tels que les tirets ou les points lorsqu’ils apparaissent à l’intérieur des mots. Pour chaque couche du paquet, une phrase distincte est construite, et le nom du protocole est explicitement ajouté afin d’enrichir le contexte lexical. Le modèle considère ainsi l’ensemble du paquet comme une unité globale, tout en tirant parti de la séparation entre les phrases pour structurer l’information.
...
Les tokens obtenus sont ensuite convertis en identifiants (token IDs), qui sont fournis au modèle. Celui-ci produit en sortie des vecteurs d’embeddings, représentant chaque token. Comme un mot peut être composé de plusieurs tokens, il est nécessaire d’agréger ces représentations. La méthode la plus simple consiste à effectuer la moyenne des embeddings des tokens associés à un même mot.
...
Comme il existe plusieurs méthodes, chacune présentant ses avantages, nous proposons dans le chapitre suivant une analyse empirique afin d’évaluer les résultats de ces différents modèles.
